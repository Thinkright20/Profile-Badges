...
This repository includes a collection of every single badge on GitHub, and how to obtain each of them. Some are obtainable, not obtainable, or still in beta testing. This repository orders the badges from `Very Easy` to `Easy` to `Medium` and finally `Hard`.
...
> Now today, there is a total of 11 badges on GitHub, some with a bronze, silver, and gold tier too.
